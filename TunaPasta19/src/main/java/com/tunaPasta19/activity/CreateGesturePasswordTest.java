...
import com.tunaPasta19.widget.LockPatternView.DisplayMode;
...
public class CreateGesturePasswordTest extends Activity implements
    ...
    // 底部两个按钮的状态枚举：就这样，只要LockPatternView的状态一发生改变，
    // 就会动态改变底部两个Button的文字和状态。很简洁，逻辑性很强。
    enum LeftButtonMode {
        // 取消
    ...
    // 值得学习的地方是(代码来自设置应用中)：在创建解锁图案时的枚举使用，原生代码中使用了很多枚举，
    // 将绘制图案时的状态、底部两个按钮状态、顶部一个TextView显示的提示文字都紧密的联系起来。
    // 因此，只用监听LockPatternView动态变化，对应改变底部Button和顶部TextView的状态即可实现联动，
    // 简单的方法可以实现很多代码才能实现的逻辑，个人很喜欢。
    // ①全局的状态：
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.creategesturepasswordtest);
        // 初始化演示动画
        ...
        mLockPatternView = this.findViewById(R.id.gesturepwd_create_lockview);

        mHeaderText = findViewById(R.id.gesturepwd_create_text);
        mLockPatternView.setOnPatternListener(mChooseNewLockPatternListener);
        mLockPatternView.setTactileFeedbackEnabled(true);

        mFooterRightButton = this.findViewById(R.id.right_btn);
        mFooterLeftButton = this.findViewById(R.id.reset_btn);
    ...
    private void saveChosenPatternAndFinish() {
        DataTrans.getInstance().getLockPatternUtils().saveLockPattern(mChosenPattern);
        showToast("密码设置成功");
        startActivity(new Intent(this, UnlockGesturePasswordTest.class));
        finish();
    }
}
