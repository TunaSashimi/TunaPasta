...
public class Rotate3DEffectItemAnimation {
	...
	public static void startAnimationsIn(ViewGroup viewgroup, int durationMillis) {
		for (int i = 0; i < viewgroup.getChildCount(); i++) {
			ImageButton imagebuttonin = (ImageButton) viewgroup.getChildAt(i);
			imagebuttonin.setVisibility(View.VISIBLE);
			imagebuttonin.setClickable(true);
			imagebuttonin.setFocusable(true);
			MarginLayoutParams mlp = (MarginLayoutParams) imagebuttonin.getLayoutParams();
			...
			imagebuttonin.startAnimation(animation);
		}
	}
	...
	public static void startAnimationsOut(ViewGroup viewgroup, int durationMillis) {
		for (int i = 0; i < viewgroup.getChildCount(); i++) {
			...
			final ImageButton imagebuttonout = (ImageButton) viewgroup.getChildAt(i);
			MarginLayoutParams mlp = (MarginLayoutParams) imagebuttonout.getLayoutParams();
			Animation animation = new TranslateAnimation(0F, mlp.rightMargin - xOffset, 0F, yOffset + mlp.bottomMargin);

			//是否停留在动画最后的状态,因为没有对齐,所以只好设置为false
			...
			animation.setAnimationListener(new Animation.AnimationListener() {
				...
				public void onAnimationEnd(Animation arg0) {
					imagebuttonout.setClickable(false);
					imagebuttonout.setFocusable(false);
					imagebuttonout.setVisibility(View.GONE);
				}
			});
			imagebuttonout.startAnimation(animation);
		}
	}
