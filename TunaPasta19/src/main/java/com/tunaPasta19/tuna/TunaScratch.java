 ...
 * @Description
 */
public class TunaScratch extends TunaView{
	...
	public TunaScratch(Context context) {
		this(context, null);
	}

	public TunaScratch(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public TunaScratch(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);

		tunaTag = TunaScratch.class.getSimpleName();

		//
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.TunaScratch);
		int tunaScratchSrcId = typedArray.getResourceId(R.styleable.TunaScratch_tunaScratchSrc, -1);
		if (tunaScratchSrcId > -1) {
			tunaScratchSrcBitmap = BitmapFactory.decodeResource(getResources(), tunaScratchSrcId);
		}

		tunaScratchRadius = (int) typedArray.getDimension(R.styleable.TunaScratch_tunaScratchRadius, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics()));
		tunaScratchCoverColor = typedArray.getColor(R.styleable.TunaScratch_tunaScratchCoverColor, Color.parseColor("#c0c0c0"));

		tunaScratchCoverStrokeWidth = (int) typedArray.getDimension(R.styleable.TunaScratch_tunaScratchCoverStrokeWidth, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, getResources().getDisplayMetrics()));
		tunaScratchText = typedArray.getString(R.styleable.TunaScratch_tunaScratchText);

		tunaScratchTextSize = (int) typedArray.getDimension(R.styleable.TunaScratch_tunaScratchTextSize, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 22, getResources().getDisplayMetrics()));
		tunaScratchTextColor = typedArray.getColor(R.styleable.TunaScratch_tunaScratchTextColor, Color.TRANSPARENT);
