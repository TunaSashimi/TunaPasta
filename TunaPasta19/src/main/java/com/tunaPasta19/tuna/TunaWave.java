 ...
 * @Description
 */
public class TunaWave extends TunaView {
    ...
    public TunaWave(Context context) {
        this(context, null);
    }

    public TunaWave(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TunaWave(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        tunaTag = TunaWave.class.getSimpleName();

        // 将dp转化为px，用于控制不同分辨率上移动速度基本一致
        mXOffsetSpeedOne = dpToPx(context, TRANSLATE_X_SPEED_ONE);
        mXOffsetSpeedTwo = dpToPx(context, TRANSLATE_X_SPEED_TWO);
