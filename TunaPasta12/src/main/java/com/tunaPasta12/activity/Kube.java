...
public class Kube extends Activity implements KubeRenderer.AnimationCallback {
    ...
    private GLWorld makeGLWorld()
    {
        ...
        mCubes[0]  = new KubeCube(world, c0, c4, c0, c1, c5, c1);
        mCubes[1]  = new KubeCube(world, c2, c4, c0, c3, c5, c1);
        mCubes[2]  = new KubeCube(world, c4, c4, c0, c5, c5, c1);
        // top middle, left to right
        mCubes[3]  = new KubeCube(world, c0, c4, c2, c1, c5, c3);
        mCubes[4]  = new KubeCube(world, c2, c4, c2, c3, c5, c3);
        mCubes[5]  = new KubeCube(world, c4, c4, c2, c5, c5, c3);
        // top front, left to right
        mCubes[6]  = new KubeCube(world, c0, c4, c4, c1, c5, c5);
        mCubes[7]  = new KubeCube(world, c2, c4, c4, c3, c5, c5);
        mCubes[8]  = new KubeCube(world, c4, c4, c4, c5, c5, c5);
        // middle back, left to right
        mCubes[9]  = new KubeCube(world, c0, c2, c0, c1, c3, c1);
        mCubes[10] = new KubeCube(world, c2, c2, c0, c3, c3, c1);
        mCubes[11] = new KubeCube(world, c4, c2, c0, c5, c3, c1);
        // middle middle, left to right
        mCubes[12] = new KubeCube(world, c0, c2, c2, c1, c3, c3);
        mCubes[13] = null;
        mCubes[14] = new KubeCube(world, c4, c2, c2, c5, c3, c3);
        // middle front, left to right
        mCubes[15] = new KubeCube(world, c0, c2, c4, c1, c3, c5);
        mCubes[16] = new KubeCube(world, c2, c2, c4, c3, c3, c5);
        mCubes[17] = new KubeCube(world, c4, c2, c4, c5, c3, c5);
        // bottom back, left to right
        mCubes[18] = new KubeCube(world, c0, c0, c0, c1, c1, c1);
        mCubes[19] = new KubeCube(world, c2, c0, c0, c3, c1, c1);
        mCubes[20] = new KubeCube(world, c4, c0, c0, c5, c1, c1);
        // bottom middle, left to right
        mCubes[21] = new KubeCube(world, c0, c0, c2, c1, c1, c3);
        mCubes[22] = new KubeCube(world, c2, c0, c2, c3, c1, c3);
        mCubes[23] = new KubeCube(world, c4, c0, c2, c5, c1, c3);
        // bottom front, left to right
        mCubes[24] = new KubeCube(world, c0, c0, c4, c1, c1, c5);
        mCubes[25] = new KubeCube(world, c2, c0, c4, c3, c1, c5);
        mCubes[26] = new KubeCube(world, c4, c0, c4, c5, c1, c5);

        // paint the sides
        int i, j;
        // set all faces black by default
        for (i = 0; i < 27; i++) {
            KubeCube cube = mCubes[i];
            if (cube != null) {
                for (j = 0; j < 6; j++)
        ...
        // paint top
        for (i = 0; i < 9; i++)
            mCubes[i].setFaceColor(KubeCube.kTop, orange);
        // paint bottom
        for (i = 18; i < 27; i++)
            mCubes[i].setFaceColor(KubeCube.kBottom, red);
        // paint left
        for (i = 0; i < 27; i += 3)
            mCubes[i].setFaceColor(KubeCube.kLeft, yellow);
        // paint right
        for (i = 2; i < 27; i += 3)
            mCubes[i].setFaceColor(KubeCube.kRight, white);
        // paint back
        for (i = 0; i < 27; i += 9)
            for (j = 0; j < 3; j++)
                mCubes[i + j].setFaceColor(KubeCube.kBack, blue);
        // paint front
        for (i = 6; i < 27; i += 9)
            for (j = 0; j < 3; j++)
                mCubes[i + j].setFaceColor(KubeCube.kFront, green);
 ...
 //    			newPermutation[i] = mCurrentLayerPermutation[mPermutation[i]];
    ...
    KubeCube[] mCubes = new KubeCube[27];
