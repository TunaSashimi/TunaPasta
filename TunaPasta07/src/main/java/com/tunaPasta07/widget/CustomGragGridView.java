...
import com.tunaPasta07.activity.CustomGragGridViewTest.DragGridAdapter;

public class CustomGragGridView extends GridView {
	...
	public CustomGragGridView(Context context) {
		super(context);
	}

	public CustomGragGridView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}

	public CustomGragGridView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			...
			Log.i(TAG, "[onInterceptTouchEvent] dragSrcPosition:"
					+ dragSrcPosition + ",getFirstVisiblePosition():"
					+ getFirstVisiblePosition());
			// getFirstVisiblePosition()返回第一个dispaly在界面的view在adapter的位置
			// 可能是0,也可能是4
			...
			Log.i(TAG, "[onInterceptTouchEvent] [x:" + x + ",y:" + y
					+ "],[rawX:" + ev.getRawX() + ",rawY:" + ev.getRawY() + "]");
			Log.i(TAG, "[onInterceptTouchEvent] [dragPointX:" + dragPointX
					+ ",dragPointY:" + dragPointY + "],[dragOffsetX:"
	...
	public void onDrop(int x, int y) {
		...
		if (dragPosition != dragSrcPosition && dragPosition > -1
			...
			System.out.println("srcPosition=" + dragSrcPosition
