...
public class ScrollEffectLinearLayout extends LinearLayout {
	
	private Scroller mScroller = null;
	public boolean isCover = true;
	private float x=0;
	
	public ScrollEffectLinearLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
...
//		也就是说这个类实际上就是相当于一个代理，值是 为了给后面视图移动添加一些动画效果。所以单独调用startScroll()而不重写computeScroll()方法是不会看到任何效果的。这两者 必须配合使用,才能有移动的时候的动画效果。
	...
	public void scrollToTarget() {
		int fx=(int) (getWidth()*0.9);//移动的区域为总区域的90%
		if (!isCover) {
			setClick(true);
