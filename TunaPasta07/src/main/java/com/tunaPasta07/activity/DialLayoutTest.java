...
import com.tunaPasta07.widget.DialLayoutAnimation;

/**
 * 主Activity,实现各点击事件，这里只实现了两个
 */
public class DialLayoutTest extends Activity {
		...
		menu.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (isLevel3Show) {
					...
					DialLayoutAnimation.startAnimationOUT(level3, 500, 0);
				} else {
					// 否则就显示3级导航菜单
					DialLayoutAnimation.startAnimationIN(level3, 500);
				}
		...
		home.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (!isLevel2Show) {
					...
					DialLayoutAnimation.startAnimationIN(level2, 500);
				} else {
					// 否则分两种情况：
					if (isLevel3Show) {
						// 如果3级布局显示,隐藏3级导航菜单
						DialLayoutAnimation.startAnimationOUT(level3, 500, 0);
						// 也隐藏2级导航菜单
						DialLayoutAnimation.startAnimationOUT(level2, 500, 500);
						// 同时每次点击都改变3级布局判断变量的布尔值类型
						isLevel3Show = !isLevel3Show;
					} else {// 如果只有2级布局显示
							// 就只隐藏2级导航菜单
						DialLayoutAnimation.startAnimationOUT(level2, 500, 0);
					}
				}
