...
public class P00_WelcomAct extends Activity {
                ...
                try {
                    ...
                    if (!started) {
                        startActivity(new android.content.Intent(
                                P00_WelcomAct.this, P01_EntryAct.class));
                        P00_WelcomAct.this.finish();
                    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            started = true;
            startActivity(new android.content.Intent(P00_WelcomAct.this, P01_EntryAct.class));
            P00_WelcomAct.this.finish();
        }
...
class OpenMoveView extends View {
    ...
    public OpenMoveView(Context context) {
        ...
        backfirst = BitmapFactory.decodeResource(context.getResources(), R.drawable.welcome0);
        backfirst = Bitmap.createScaledBitmap(backfirst, screenWidth, 802, false);
        // 得到backmiddle,并把backfirst图像压缩成~~屏幕宽度*802
        backmiddle = BitmapFactory.decodeResource(context.getResources(), R.drawable.welcome1);
        backmiddle = Bitmap.createScaledBitmap(backmiddle, screenWidth, 802,
                false);
        // 得到backlast,原图大小1920*952,要缩小,不然虚拟机内存溢出
        backlast = BitmapFactory.decodeResource(context.getResources(),R.drawable.welcome2);
        ...
        tmpCanvas.drawText("NemesisView", 20, 420, paint);
        // 使用SRC_IN
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i <= 15; i++) {
                        Thread.sleep(100);
                        back = Bitmap.createScaledBitmap(back, screenHeight - 6
                                * i, screenWidth - 6 * i, false);
                        startX += 5;
                        startY += 5;
                        width -= 1;
                        height -= 2;
