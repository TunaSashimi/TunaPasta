...
import com.tunaPasta05.entity.WaterFallTestFlowTag;
import com.tunaPasta05.widget.WaterFallTestFlowImageView;
import com.tunaPasta05.widget.WaterFallTestScrollView;
import com.tunaPasta05.widget.WaterFallTestScrollView.OnScrollListener;
...
public class WaterFallTest extends Activity {
    ...
    private ArrayList<LinearLayout> waterfall_layout = new ArrayList();    //列布局集合
    ...
    private void InitLayout() {
        ...
        try {                                                                            // 加载所有图片路径,先转成字符串数组,再转成List
            image_filenames = Arrays.asList(asset_manager.list("images"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void AddImage(String filename, int rowIndex, int id) {
        ...
        flowtag.fileName = "images/" + filename;
