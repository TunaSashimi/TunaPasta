...
public class MyImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > PicActivity.screenWidth) {
		// 如果图宽大于屏宽，就不用水平居中
		center(false, true);
		} else {
		center(true, true);
	...
	private void arithScaleRate() {
		float scaleWidth = PicActivity.screenWidth / (float) imageWidth;
		float scaleHeight = PicActivity.screenHeight / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,PicActivity.screenWidth / 2f, PicActivity.screenHeight / 2f);
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = PicActivity.screenWidth - width;
		float fill_height = PicActivity.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( PicActivity.screenWidth/2f)/imageWidth;
