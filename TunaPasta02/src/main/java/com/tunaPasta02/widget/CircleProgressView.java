...
public class CircleProgressView extends View {
	...
	public static final int[] CircleProgressBar = { 0x7f010000, 0x7f010001,
	...
	public CircleProgressView(Context context, AttributeSet attrs) {
		...
		TypedArray array = context.obtainStyledAttributes(attrs,CircleProgressBar);

		mMaxProgress = array.getInteger(CircleProgressBar_max,DEFAULT_MAX_VALUE); // 获取进度条最大值

		boolean bFill = array.getBoolean(CircleProgressBar_fill,DEFAULT_FILL_MODE); // 获取填充模式
		
		int paintWidth = array.getInt(CircleProgressBar_Paint_Width,DEFAULT_PAINT_WIDTH); // 获取画笔宽度
		mCircleAttribute.setFill(bFill);
		if (bFill == false) {
			mCircleAttribute.setPaintWidth(paintWidth);
		}

		int paintColor = array.getColor(CircleProgressBar_Paint_Color,DEFAULT_PAINT_COLOR); // 获取画笔颜色

		mCircleAttribute.setPaintColor(paintColor);

		mCircleAttribute.mSidePaintInterval = array.getInt(CircleProgressBar_Inside_Interval, DEFAULT_INSIDE_VALUE);// 圆环缩进距离
