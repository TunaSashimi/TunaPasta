...
//个人觉得比较有用的就是加密这一块了，为了以后方便使用，
//我把图案加密和字符加密分成两个工具类：LockPatternUtils和LockPasswordUtils两个文件，
//本文使用到的是LockPatternUtils。
//其实所谓的图案加密也是将其通过SHA-1加密转化成二进制数再保存到文件中
//(原生系统保存在/system/目录下，我这里没有权限，就保存到本应用目录下)，
//解密时，也是将获取到用户的输入通过同样的方法加密，再与保存到文件中的对比，相同则密码正确，
//不同则密码错误。关健代码就是以下4个函数：
//patternToString,saveLockPattern,patternToHash,checkPattern
